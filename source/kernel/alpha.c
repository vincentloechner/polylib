...
/* result =  1 --> P1 > P2        */
/* INDEX  = 1 .... Dimension      */
int PolyhedronLTQ(Polyhedron *Pol1, Polyhedron *Pol2, int INDEX, int PDIM,
                  int NbMaxConstrs) {
  ...
  if (emptyQ(Q))
    ...
    for (i = 0; i < Q1->NbConstraints; i++) {
      if ((value_one_p(Q1->Constraint[i][0])) &&
          (value_pos_p(Q1->Constraint[i][INDEX]))) {
    ...
    for (i = 0; i < Q2->NbConstraints; i++) {
      if ((value_one_p(Q2->Constraint[i][0])) &&
          (value_neg_p(Q2->Constraint[i][INDEX]))) {
...
int GaussSimplify(Matrix *Mat1, Matrix *Mat2) {
      ...
      for (i = 0; i < NbRows; i++) /* Zero out the rest of the column */
        if (i != Rank) {
          if (value_notzero_p(Mat1->p[i][j])) {
            ...
            value_oppose(a1, a1);
            Vector_Combine(Mat1->p[i], Mat1->p[Rank], Mat1->p[i], a2, a1,
                           NbCols);
  ...
  if (Mat2) { /* Mat2 is a transformation matrix  (i,j->f(i,j))....
    ...
    for (k = 0; k < Rank; k++) {
      j = column_index[k];
      for (i = 0; i < (Mat2->NbRows - 1);
           i++) { /* all but the last row 0...0 1 */
        if ((i != j) && value_notzero_p(Mat2->p[i][j])) {
          ...
          value_oppose(a1, a1);
          if (value_one_p(a2)) {
            Vector_Combine(Mat2->p[i], Mat1->p[k], Mat2->p[i], a2, a1, NbCols);
          ...
          for (n = j + 1; n < (NbCols - 1); n++) {
            if (value_notzero_p(Mat2->p[i][n])) { /* i' depends on some n */
              value_set_si(tmp, 1);
              Vector_Combine(Mat2->p[i], Mat1->p[k], Mat2->p[i], tmp, tmp,
                             NbCols);
              break;
 ...
 */
int PolyhedronTSort(Polyhedron **L, unsigned int n, unsigned int index,
                    unsigned int pdim, int *time, int *pvect,
                    unsigned int MAXRAYS) {

  unsigned int const nbcells =
      ((n * (n - 1)) >> 1); /* Number of memory cells
