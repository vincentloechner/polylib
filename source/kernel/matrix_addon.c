...
void constraintsView_Free(Matrix *M) { free(M); }
...
void mpolyhedron_simplify(Matrix *polyh) {
  ...
  for (i = 0; i < polyh->NbRows; i++) {
    ...
    printf(" gcd[%d] = ", i);
    value_print(stdout, VALUE_FMT, cur_gcd);
    printf("\n");
    Vector_AntiScale(polyh->p[i] + 1, polyh->p[i] + 1, cur_gcd,
                     polyh->NbColumns - 1);
  }
  value_clear(cur_gcd);
 ...
 */
void eliminate_var_with_constr(Matrix *Eliminator, unsigned int eliminator_row,
                               Matrix *Victim, unsigned int victim_row,
  ...
  if (value_notzero_p(Victim->p[victim_row][var_to_elim + 1])) {
    ...
    value_division(mul_b, cur_lcm, Victim->p[victim_row][var_to_elim + 1]);
 ...
 */
void Matrix_copySubMatrix(Matrix *M1, unsigned int sr1, unsigned int sc1,
                          unsigned int nbR, unsigned int nbC, Matrix *M2,
