...
unsigned int *permutation_inverse(unsigned int *perm, unsigned int nb_elems) {
  ...
  unsigned int *inv_perm =
      (unsigned int *)malloc(sizeof(unsigned int) * nb_elems);
...
Matrix *mtransformation_permute(Matrix *transf, unsigned int *permutation) {
  ...
  for (i = 0; i < transf->NbRows; i++) {
    for (j = 0; j < transf->NbRows; j++) {
      value_assign(permuted->p[permutation[i]][permutation[j]],
                   transf->p[i][j]);
    }
  }
  return permuted;
...
unsigned long long int eliminable_vars(Matrix *Eqs, unsigned start,
  ...
  if (Eqs->NbRows > nb_vars) {
    ...
    OverConstrained =
        Constraints2Polyhedron(Eqs2, Eqs->NbColumns * Eqs->NbColumns);
    Matrix_Free(Eqs2);
  ...
  */
  for (combination = ((unsigned long long int)1 << (Eqs->NbRows)) - 1;
       (combination < ((unsigned long long int)1 << nb_vars)); combination++) {
...
unsigned int *find_a_permutation(Matrix *Eqs, unsigned int nb_parms) {
  ...
  unsigned int *permutation =
      (unsigned int *)malloc(sizeof(unsigned int) * Eqs->NbColumns - 1);
  ...
  for (i = 0; i < nb_vars; i++) {
    ...
    /* if not, put the variables at the end */
      ...
      permutation[i] = Eqs->NbRows + nb_parms + i - k;
    combination >>= 1;
  }
...
unsigned int *permutation_for_full_dim2(unsigned int *vars_to_keep,
  ...
  unsigned int i;
  int cur_keep = 0,
      cur_go = 0; /*current number of variables to eliminate/keep*/
  ...
  for (i = 0; i < nb_parms; i++)
    permutation[i + nb_vars_parms - nb_parms] =
        i + nb_vars_parms - nb_parms - nb_keep;
