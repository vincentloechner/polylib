...
void Constraints_removeElimCols(Matrix *M, unsigned int nbVars,
  ...
  for (i = 0; i < M->NbRows; i++) {
    ...
    for (j = 0; j < M->NbColumns - 2 - nbVars; j++) {
      if (j != elimParms[k + 1]) {
        value_assign((*newM)->p[i][j - k + nbVars + 1],
                     M->p[i][j + nbVars + 1]);
      } else {
        k++;
      }
...
void Constraints_fullDimensionize(Matrix **M, Matrix **C, Matrix **VL,
                                  ...
                                  unsigned int **elimParms, int maxRays) {
  unsigned int i, j;
...
void Lattice_extractSubLattice(Matrix *lat, unsigned int k, Matrix **subLat) {
  ...
  if (k == lat->NbRows - 1) {
    if (*subLat == NULL) {
      ...
      Matrix_copySubMatrix(lat, 0, 0, lat->NbRows, lat->NbColumns, (*subLat), 0,
                           0);
    }
    return;
  }
...
Matrix *affine_periods(Matrix *M, Matrix *d) {
  ...
  for (i = 0; i < M->NbColumns; i++)
    for (j = 0; j < M->NbColumns; j++)
      if (i == j)
        value_assign(S->p[i][j], periods[j]);
      else
        value_set_si(S->p[i][j], 0);

  /* 3- clean up */
  for (i = 0; i < M->NbColumns; i++)
    value_clear(periods[i]);
  free(periods);
  return S;
} /* affine_periods */
...
Matrix *Constraints_Remove_parm_eqs(Matrix **M1, Matrix **Ctxt1,
                                    int renderSpace, unsigned int **elimParms) {
  ...
  EqsMTmp =
      Matrix_Alloc(Ctxt->NbRows - nbEqsCtxt - nbTautoCtxt, Ctxt->NbColumns);
  k = 0;
  for (i = 0; i < Ctxt->NbRows; i++) {
    if (value_notzero_p(Ctxt->p[i][0])) {
...
Matrix *full_dimensionize(Matrix const *M, int nbParms,
  ...
  *validityLattice = Matrix_Alloc(nbParms + 1, nbParms + 1);
  for (i = 0; i < nbParms; i++) {
    for (j = 0; j < nbParms; j++)
      value_assign((*validityLattice)->p[i][j], WVL->p[i][j]);
