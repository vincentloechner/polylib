...
Matrix *CalcPolyhedronBase( Polyhedron *P )
{
  ...
  if( emptyQ(P) )
    return( Matrix_Alloc( 0, P->Dimension ) ); */
/* pas de vect. gen */                                           /*
...
/* on ne peut pas allouer 0 lignes ! */                          /*
B->NbRows = 0;
return( B );
}
...
void new_eadd(evalue *e1, evalue *res) {
  ...
  /* ######### add polynomial or periodic to constant #############
    ...
    if ((e1->x.p->type != res->x.p->type)) {
      ...
      /* ### adding evalues of different position (i.e function of different
         unknowns to case are possible  ### */

      if (res->x.p->type ==
          polynomial) { /*  ### res and e1 are polynomials
                            add e1 to the constant term of res */
    ...
    /* ### add two periodics of the same pos (unknown) but whith different sizes
       (periods) ### */
    else if (res->x.p->type == periodic) {
...
/* Remove validite domains and correspending ehrhart polynomials whitch are
 redundant after the enumeration of a polyhedron */
  ...
  while (Polun->next != NULL) {
    ...
    for (en1 = Polun->pt; en1; en1 = enext) {
      ...
      for (en2 = (Polun->next)->pt; en2; en2 = en2->next) {
        d = DomainIntersection(en1->ValidityDomain, en2->ValidityDomain,
                               MAXRAYS);
...
Enumeration *Polyhedron_Image_Enumerate(Polyhedron *D, Polyhedron *C, Matrix *T,
  ...
  if (d->NbRows == 0) {
    if (emptyQ(D)) {
      ...
      printf("        Ker(A)=0  implys directly Enumeration on input "
             "polyhedron\n\n");
