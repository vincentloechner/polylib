...
Polyhedron *LexSmaller(Polyhedron *P, Polyhedron *D, unsigned dim,
  ...
  if (D_extra)
    cur_element =
        Matrix_Alloc(P->NbConstraints + D->NbConstraints + nb_new_parms,
  ...
  for (i = 0; i < P->NbConstraints; i++) {
    ...
    Vector_Copy(P->Constraint[i] + 1 + nb_vars + P_extra,
                cur_element->p[i] + 1 + nb_vars + P_extra + D_extra +
                    nb_new_parms,
                nb_parms + 1);
  }
  ...
  if (D_extra)
    ...
    C_times_J =
        Matrix_Alloc(C->NbConstraints + D->NbConstraints, D->Dimension + 2);
  ...
  for (i = 0; i < C->NbConstraints; i++) {
    ...
    Vector_Copy(C->Constraint[i] + 1, C_times_J->p[i] + 1 + nb_new_parms,
                nb_parms + 1);
  }
 ...
 */
Enumeration *Polyhedron_Ranking(Polyhedron *P, Polyhedron *C,
                                unsigned MAXRAYS) {
  return Polyhedron_LexSmallerEnumerate(P, P, P->Dimension - C->Dimension, C,
                                        MAXRAYS);
}
