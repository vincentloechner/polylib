...
def main():
  ...
  univ = LBL("{(x,y)|}")
  assert str(univ) == 'LBL("{(i, j) | }")'
  empty = LBL("{(x,y)|1 < 0}")
  assert str(empty) == 'LBL("{(_, _) | <empty>}")'
  print("OK")
  ...
  b = LBL("{(i,j) | j < 5}")
  assert str(a) == 'LBL("{(2i, j) | i <= 5, -2i+j - 1 >= 0, 3i-j >= 0}")'
  assert str(b) == 'LBL("{(i, j) | j <= 4}")'
  ...
  # union
  u = a + b
  assert str(u) == """LBL("{(2i, j) | i <= 5, -2i+j - 1 >= 0, 3i-j >= 0}") + \\
LBL("{(i, j) | j <= 4}")"""
  assert u == b.union(a) == a.union(b)

  # difference
  d = a - b
  assert str(d) == 'LBL("{(2i, j) | i <= 5, -2i+j - 1 >= 0, 3i-j >= 0, j >= 5}")'
  assert d == a.difference(b)

  # intersection
  i = a * b
  assert str(i) == 'LBL("{(2i, j) | -2i+j - 1 >= 0, 3i-j >= 0, j <= 4}")'
  ...
  # another difference test, checking that the result is simple
  a1 = LBL("{(2i, 2i+j) | 1 <= i <= 5, 1 <= j <= i}")
  b1 = LBL("{(i,j) | 0 <= i <= 6, 0 <= j <= 8}")
  assert str(a1-b1) == 'LBL("{(2i, j) | i <= 5, -2i+j - 1 >= 0, 3i-j >= 0, j >= 9}")'
  ...
  assert str(t) == 'Transfo("i, j -> 3i+1, i+j")'
  ...
  # composition
  t2 = Transfo("i, j -> j, i")
  assert str(t2 * t) == 'Transfo("i, j -> i+j, 3i+1")'

  # invert
  t_inv = t.inverse()
  assert str(t_inv) == 'Transfo("i, j -> i-1, -i+3j+1")'
